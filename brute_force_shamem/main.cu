...
__global__ void detectCollisionCUDA(
    ...
    __shared__ int sphereNumShaTrue;
    __shared__ int triangleNumShaTrue;
    if (tx == 0 && ty == 0) {
        if (blockIdx.x == (gridDim.x - 1)) {
            sphereNumShaTrue = sphereNum - sphereNumSha * (gridDim.x - 1);
        }
        else
            sphereNumShaTrue = sphereNumSha;

        if (blockIdx.y == (gridDim.y - 1)) {
            triangleNumShaTrue = triangleNum - triangleNumSha * (gridDim.y - 1);
        }
        else
            triangleNumShaTrue = triangleNumSha;
    } 
    __syncthreads();
    
    int inBlkIdx = tx + ty * sphereNumSha;
    
    extern __shared__ sphere buffer[];
    sphere *sphereSha = (sphere *) &buffer[0];
    triangle *triangleSha = (triangle *) &buffer[sphereNumShaTrue];
    
    if (inBlkIdx < sphereNumShaTrue)
        sphereSha[inBlkIdx] = sphereD[sphereSegId * sphereSegSize +
                                      bx * sphereNumSha + inBlkIdx];
    else if (inBlkIdx < (sphereNumShaTrue + triangleNumShaTrue))
        triangleSha[inBlkIdx - sphereNumShaTrue] = triangleD[triangleSegId * triangleSegSize + 
                                      by * triangleNumSha + inBlkIdx - sphereNumShaTrue];
    __syncthreads();
     
    int outBlkIdx_x = tx + bx * sphereNumSha;
    int outBlkIdx_y = ty + by * triangleNumSha;

    if(outBlkIdx_x < sphereNum && outBlkIdx_y < triangleNum) {    
        if (detectCollision(sphereSha[tx], triangleSha[ty])) {   
            int i = sphereNum * outBlkIdx_y + outBlkIdx_x;
            sphereCllsnIdD[i] = sphereSha[tx].id;
            triangleCllsnIdD[i] = triangleSha[ty].id;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for (int j = 0; j < triangleSegNum; j++) {
        for (int i = 0; i < sphereSegNum; i++) { 
            ...
            // *********************************************************************************
            int sphereNumSha = 16;
            int triangleNumSha = 16;
            dim3 dimBlock(sphereNumSha, triangleNumSha);
            dim3 dimGrid((trueSphereNum + dimBlock.x - 1) / dimBlock.x, 
                         (trueTriangleNum + dimBlock.y - 1) / dimBlock.y);
            // Shared memory
            int shaMemSize = sphereNumSha * sizeof(sphere) + triangleNumSha * sizeof(triangle);

            detectCollisionCUDA<<<dimGrid, dimBlock, shaMemSize>>>(
                sphereD, triangleD, i, j, sphereSegSize, triangleSegSize, 
                trueSegSize, trueSphereNum, trueTriangleNum, sphereCllsnIdD, triangleCllsnIdD,
                sphereNumSha, triangleNumSha);
