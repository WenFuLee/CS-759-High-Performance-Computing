...
int main(int argc, char *argv[])
{
    ...
    for (int k = 0; k < totalBins; k++) {
        for (int i = 0; i < binToTriangles[k].size(); i++) {
            for (int j = 0; j < binToSpheres[k].size(); j++) {
                if (detectCollision(binToSpheres[k][j], binToTriangles[k][i])) {
                    sphereCllsnID.push_back(binToSpheres[k][j].id);
                    triangleCllsnID.push_back(binToTriangles[k][i].id);
                }
            }
        }
    }
	...
	pair<int, int> index_pair = make_pair(sphereCllsnID[i], triangleCllsnID[i]);
	if (flags.find(index_pair) == flags.end()) {
            flags.insert(index_pair);
	    sCllsnID.push_back(sphereCllsnID[i]);
	    tCllsnID.push_back(triangleCllsnID[i]);
	}
    }
